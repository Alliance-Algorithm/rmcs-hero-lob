...
inline std::array<std::uint8_t, g_exp_table_size> g_exp_table{};
inline std::array<std::uint8_t, g_gf_order + 1> g_log_table{};
inline bool g_gf256_ready = false;

inline void init_gf256() {
    if (g_gf256_ready)
        return;
    std::uint16_t value = 1;
    for (std::uint16_t i = 0; i < g_gf_order; ++i) {
        g_exp_table[i] = static_cast<std::uint8_t>(value);
        g_log_table[value] = static_cast<std::uint8_t>(i);
        value <<= 1;
        if (value >= 256)
            value ^= 0x11D;
    }
    for (std::uint16_t i = g_gf_order; i < g_exp_table_size; ++i)
        g_exp_table[i] = g_exp_table[i - g_gf_order];
    g_gf256_ready = true;
}

inline std::uint8_t gf_add(std::uint8_t a, std::uint8_t b) { return static_cast<std::uint8_t>(a ^ b); }
inline std::uint8_t gf_sub(std::uint8_t a, std::uint8_t b) { return static_cast<std::uint8_t>(a ^ b); }

inline std::uint8_t gf_mul(std::uint8_t a, std::uint8_t b) {
    if (a == 0 || b == 0)
        return 0;
    init_gf256();
    return g_exp_table[(g_log_table[a] + g_log_table[b]) % g_gf_order];
}
...
inline std::uint8_t gf_inv(std::uint8_t a) {
    ...
    init_gf256();
    return g_exp_table[g_gf_order - g_log_table[a]];
}

inline std::vector<std::uint8_t> build_cauchy_row(std::uint8_t row_idx, std::uint8_t k_data) {
...
inline std::uint8_t make_fec_status(const std::uint8_t k_prime, const bool is_last) {
    return static_cast<std::uint8_t>((k_prime << kKPrimeShift) | (is_last ? kStatusEnd : kStatusFec));
}

inline std::vector<std::array<std::uint8_t, kPacketSize>>
    packetize_jpeg(
        const std::span<const std::uint8_t> jpeg_bytes, const std::uint8_t image_sequence,
        const std::uint8_t message_type = kMessageTypeImage) {
...
inline std::vector<PayloadBuffer>
    ...
    for (std::uint8_t fec_idx = 0; fec_idx < r_fec; ++fec_idx) {
        const auto coeffs = detail::build_cauchy_row(fec_idx, k_data);
        for (std::size_t byte = 0; byte < kPayloadSize; ++byte) {
            std::uint8_t sum = 0;
            for (std::uint8_t data_idx = 0; data_idx < k_data; ++data_idx)
                sum = detail::gf_add(sum, detail::gf_mul(coeffs[data_idx], data_payloads[data_idx][byte]));
